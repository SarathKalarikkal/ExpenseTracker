...
const updateNoTransactionMessage = () => {
  const noTransactionElement = document.querySelector(".noTransaction");
  if (incomeArray.length === 0 && expenseArray.length === 0) {
    if (!noTransactionElement) {
      const noTransaction = document.createElement("div");
      noTransaction.className = "noTransaction";
      noTransaction.innerHTML = "<p>No transactions</p>";
      transListWrapper.appendChild(noTransaction);
    }
  } else {
    if (noTransactionElement) {
      noTransactionElement.remove();
    }
  }
};
...
form.addEventListener("submit", (e) => {
  ...
  const formatedDate = `${String(newDate.getDate()).padStart(2, "0")}/${String(
    newDate.getMonth() + 1
...
function updateTotals() {
  const incomeSum = incomeArray.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
  const expenseSum = expenseArray.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
